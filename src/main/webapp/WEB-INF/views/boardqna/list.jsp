	...
	<div id="listLayout">
		...
		<center>
			<div style="width: 1400px; margin: auto;">
				<div class="name">
					<h2>
						<img src="resources/image/qna.png" alt="Q&A 개시판">
					</h2>
				</div>
					...
					<c:if test="${member.member_id != null }">
						<c:if test="${member.member_id != 'Admin'}">
							<button style="cursor: pointer" type="button" class="text"
								onClick="location.href='writeQnA'">글쓰기</button>
						</c:if>
					</c:if>
				</div>
				<br />
							...
							<c:when test="${not empty row}">
								<tr height=40; class="jyy">
									...
									<td class="list2" style="text-align: left;"><a
										href="viewQnA?bno=${row.bno}&show=Y"
				...
				<div align="center">
					<c:if test="${map.pager.curBlock > 1}">
						<a
							href="boardqna?curPage=1
                     &searchOption=${searchOption}&keyword=${keyword}
                     &search=${search}">[처음]</a>
					</c:if>
					<c:if test="${map.pager.curBlock > 1}">
						<a
							href="boardqna?curPage=${map.pager.prevPage}
                     &searchOption=${searchOption}&keyword=${keyword}
                     &search=${search}">[이전]</a>
					</c:if>
					...
					<c:if test="${map.pager.curBlock < map.pager.totBlock}">
						<a
							href="boardqna?curPage=${map.pager.nextPage}
                     &searchOption=${searchOption}&keyword=${keyword}
                     &search=${search}">[다음]</a>
					</c:if>
					<c:if
            ...
            (map.pager.totPage != map.pager.curPage)}">
						<a
							href="boardqna?curPage=${map.pager.totPage}
                     &searchOption=${searchOption}&keyword=${keyword}
                     &search=${search}">[끝]</a>
					</c:if>
				</div>
