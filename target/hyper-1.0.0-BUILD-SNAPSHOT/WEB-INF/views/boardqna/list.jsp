...
<div id="listLayout">
	...
	<center>
		...
		<div class="name">
			<h2><img src="resources/image/qna.png" alt="Q&A 개시판"> </h2>
		</div>
			...
			<!-- 일반사용자일 때 -->
				...
				<c:if test="${member.member_id != 'Admin'}">
					<button style="cursor: pointer" type="button" class="text" onClick="location.href='writeQnA'">글쓰기</button>
				</c:if>
				</c:if>
			</div>
			<br/><br/><br/>
			<table cellpadding="0" cellspacing="0" style="text-align:center;" width="100%">
				...
				<c:forEach begin="0" end="${(fn:length(map.list))}" var="i">
					...
					<tr height=40;>
						...
						<td  class="list2" style="text-align:left;"><a href="viewQnA?bno=${row.bno}&show=Y">${row.title}</a>
			...
			<div align="center">
				<c:if test="${map.pager.curBlock > 1}">
					<a href="boardqna?curPage=1
							&searchOption=${searchOption}&keyword=${keyword}
							&search=${search}">[처음]</a>
				</c:if>
				<c:if test="${map.pager.curBlock > 1}">
					<a href="boardqna?curPage=${map.pager.prevPage}
							&searchOption=${searchOption}&keyword=${keyword}
							&search=${search}">[이전]</a>
				</c:if>
			...
			<!-- 검색 조건 뷰 -->
			<form action="boardqna" class="boardsearch" align="left">
				...
				<input style="cursor: pointer" type="submit" class="check2" value="조회"/>
			</form>		
			<br/>		
				<c:if test="${map.pager.curBlock < map.pager.totBlock}">
					<a href="boardqna?curPage=${map.pager.nextPage}
							&searchOption=${searchOption}&keyword=${keyword}
							&search=${search}">[다음]</a>
				</c:if>
				<c:if test="${(map.pager.totPage > 5) && 
				(map.pager.totPage != map.pager.curPage)}">
					<a href="boardqna?curPage=${map.pager.totPage}
							&searchOption=${searchOption}&keyword=${keyword}
							&search=${search}">[끝]</a>
				</c:if>
			</div>
